...
void task(const char* threadNumber, int loopFor) {
	std::unique_lock<mutex> lock(m1, std::defer_lock); // Does not call lock on mutex m1, because used defer_lock(only own the mutex)
    ...
    //lock.try_lock(for(std::chrono::second(2)))
