...
//When using a base class pointer or reference, the compiler allows access only to members declared in the base class because type checking happens at compile time. Even if the actual object is of the derived class, child-specific members cannot be accessed unless we explicitly cast the pointer. Virtual functions work because they are declared in the base class and resolved at runtime.
